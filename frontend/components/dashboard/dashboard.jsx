...
class DashboardPage extends React.Component {
  ...
  componentDidMount() {
    ...
    this.props.fetchTransactions().then(resp => {
  ...
  componentDidUpdate() {
    ...
    if (
      this.state.numberOfOpenPositions !==
        this.state.openPositionCompanyId.length &&
      companiesLength
    ) {
      ...
      this.setState({
        numberOfOpenPositions: this.state.openPositionCompanyId.length
      });
      //remove ADDYY, price data for ADDYY requires requires an upgraded account to access
      tickerList = tickerList.filter(
        ticker => ticker !== 'ADDYY' && ticker !== 'TCEHY'
      );
      fetchBatchDayPrices(tickerList, '1D').then(resp => {
  ...
  setupMultiDayChart(allTransactions, timeframe) {
    ...
    const currentDateFourAM = new Date(
    ...
    const createHistoryEntry = currentTimeDateFormat => {
      ...
      const formattedCurrentTime = year
        .toString()
        .concat('-', month, '-', date);
    ...
    while (dayCounter < numOfDays) {
      const currentTimeEpoch =
        startTimeEpoch + dayCounter * 24 * 60 * 60 * 1000;
      const currentTimeDateFormat = new Date(currentTimeEpoch);
      // checks if date is on a weekday, skip day if it's on a weekend
      if (
        currentTimeDateFormat.getDay() > 5 ||
        currentTimeDateFormat.getDay() < 1
      ) {
        dayCounter++;
        continue;
      }
  ...
  getNewPrice(timeframe) {
    return event => {
      if (timeframe !== this.state.timeframeFocus) {
        ...
        let tickerList = this.state.openPositionCompanyId.map(
          companyId => companies[companyId].ticker
        );
        tickerList = tickerList.filter(
          ticker => ticker !== 'ADDYY' && ticker !== 'TCEHY'
        ); // price data for ADDYY and TCEHY requires an upgraded account to access
        if (timeframe === '1D') {
          this.setupOneDayChart(allTransactions);
          fetchBatchDayPrices(tickerList, '1D').then(resp => {
            this.createOneDayChartData(resp);
          ...
          fetchBatchDayPrices(tickerList, requestTimePeriod).then(resp => {
    ...
    this.state.portfolioHistroy.forEach((histroyItem, index) => {
      ...
      openPos.forEach(position => {
        ...
        if (
          ...
          data[ticker].chart[index] !== undefined
        ) {
          const closePrice =
            data[ticker].chart[index].close;
          const openPrice =
            data[ticker].chart[index].open;
      ...
      if (portfolioValueClose && portfolioValueOpen) {
        chartData.push({
          label,
          close: portfolioValueClose,
          open: portfolioValueOpen
        });
      } else {
        let close = portfolioValueClose;
        let open = portfolioValueOpen;

        if (!portfolioValueOpen) open = null;
        if (!portfolioValueClose) close = null;
    ...
    while (i < portfolioHistroy.length) {
      if (oneDataset[dataIndex]['date'] > portfolioHistroy[i].label) {
        ...
        openPos.forEach(position => {
          ...
          if (
            data[ticker] !== undefined &&
            data[ticker].chart[dataIndex] !== undefined
          ) {
            ...
            if (currentClosePrice !== null)
              latestClosePrices[ticker] = currentClosePrice;
            if (currentOpenPrice !== null)
              latestOpenPrices[ticker] = currentOpenPrice;
  ...
  render() {
    ...
    if (data.length > 0) {
      ...
      priceChangePercent = startPrice
        ? ((priceChange / startPrice) * 100).toFixed(2)
        : ((priceChange / 1) * 100).toFixed(2);
    ...
    const timeframeButtons = Object.keys(chartData).map(el => {
      return (
        <button
          ...
          id={`${graphColor.concat(
            '-',
            this.state.timeframeFocus == el
              ? `logged-in-page-timeframe-${el}`
              : ''
          )}`}
          className={'logged-in-page-timeframe-button'.concat('-', graphColor)}
          onClick={this.getNewPrice(el)}
        >
          {el}
        </button>
      );
      ...
      <div className="logged-in-page-container">
        ...
        <section className="logged-in-page-content-container">
          <section className="logged-in-page-content-left-section">
            <h2>
              {latestPrice
                ? '$'.concat(latestPrice.toLocaleString())
                : latestPrice}
            </h2>
            <h4>
              {priceChange
...
const msp = (state, ownProps) => {
  const allTransactions = Object.values(
    state.entities.transactions.allTransactions
  );
  const recentTransactions = Object.values(
    state.entities.transactions.recentTransactions
  );
